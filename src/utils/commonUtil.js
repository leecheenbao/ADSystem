// 引用所需的套件
const dotenv = require('dotenv');
const path = require('path');
const moment = require('moment-timezone');
const jwt = require('jsonwebtoken');

// 加載共用配置
dotenv.config({ path: path.resolve(process.cwd())});

// 根據 NODE_ENV 加載相應的環境文件
const envFile = `.env.${process.env.NODE_ENV || 'local'}`;
dotenv.config({ path: path.resolve(process.cwd(), envFile) });

// 設置時區
const timezone = process.env.TZ || Intl.DateTimeFormat().resolvedOptions().timeZone;

// 目前時間 (yyyy-mm-dd hh:mm:ss)
const now = () => {
  return moment(new Date()).tz(timezone).format('YYYY-MM-DD HH:mm:ss');
};

// 取得時間 (yyyy-mm-dd hh:mm:ss)
const getTime = (date) => {
  return moment(date).tz(timezone).format('YYYY-MM-DD HH:mm:ss');
};

// JWT 配置
const JWT_SECRET = process.env.JWT_SECRET || 'ICSS_SECRET';   // 確保將其設置為一個安全的密鑰
const JWT_EXPIRES_IN = process.env.JWT_EXPIRES_IN || '1h';    // Token 有效期

const generateToken = (user) => {
  return jwt.sign(
    { id: user.id, username: user.username, email: user.email, role: user.role },
    JWT_SECRET,
    { expiresIn: JWT_EXPIRES_IN }
  );
};

// 取得當天開始時間
const getStartTime = (date) => {
  return moment(date).tz(timezone).startOf('day').format('YYYY-MM-DD HH:mm:ss');
};

// 取得當天結束時間
const getEndTime = (date) => {
  return moment(date).tz(timezone).endOf('day').format('YYYY-MM-DD HH:mm:ss');
};

/**
 * 在指定時間上增加時間
 * @param {Date|string} time - 起始時間
 * @param {number} value - 要增加的時間數值
 * @param {string} unit - 時間單位 (years, months, weeks, days, hours, minutes, seconds)
 * @returns {string} - 格式化的時間字串 (YYYY-MM-DD HH:mm:ss)
 */
const addTime = (time, value, unit = 'minutes') => {
  return moment(time)
    .tz(timezone)
    .add(value, unit)
    .format('YYYY-MM-DD HH:mm:ss');
};

/**
 * 在當前時間上增加時間
 * @param {number} value - 要增加的時間數值
 * @param {string} unit - 時間單位 (years, months, weeks, days, hours, minutes, seconds)
 * @returns {string} - 格式化的時間字串 (YYYY-MM-DD HH:mm:ss)
 */
const addTimeFromNow = (value, unit = 'minutes') => {
  return addTime(new Date(), value, unit);
};

/**
 * 取得時間差
 * @param {Date|string} time - 起始時間
 * @param {Date|string} time2 - 結束時間
 * @param {string} unit - 時間單位 (years, months, weeks, days, hours, minutes, seconds)
 * @returns {number} - 時間差
 */
const getTimeDiff = (time1, time2, unit = 'seconds') => {
  return moment(time1).diff(moment(time2), unit);
};

module.exports = {
    timezone,
    now,
    getTime,
    JWT_SECRET,
    JWT_EXPIRES_IN,
    generateToken,
    getStartTime,
    getEndTime,
    addTime,
    addTimeFromNow,
    getTimeDiff,
};
